...
with tab1:
    ...
    if n_rows == 0:
        ...
        fig.update_layout(
            ...
            yaxis=dict(showgrid=True, gridcolor="rgba(128, 128, 128, 0.2)", title="Harga XAU/USD"),
            height=500,
        ...
        for model_name in selected_models:
            ...
            with col_label:
                st.markdown(
                    ...
                    f"Global<br><span style='color:var(--text-color); opacity:0.6;'>({n_rows:,} candle)</span></div>",
                    unsafe_allow_html=True
                )
            ...
            if len(event_data) > 0:
                ...
                with col_label2:
                    st.markdown(
                        f"<div style='padding-top:32px;color:{color};font-weight:700;font-size:0.9rem;'>"
                        f"Jam Rilis 📰<br><span style='color:var(--text-color); opacity:0.6;'>({len(event_data)} candle)</span></div>",
                        unsafe_allow_html=True
                    )
